...
exports.router.get('/', function (req, res) {
    ...
    async_1.waterfall([
        function (cb) {
            if (req.session) {
                if (req.query.owner)
                    req.cnn.chkQry('select distinct Conversation.id, title, ownerId, ' +
                        'lastMessage from Conversation where ownerId = ?', [req.query.owner], cb);
                else
                    req.cnn.chkQry('select distinct Conversation.id, title, ownerId, ' +
...
exports.router.post('/', function (req, res) {
    ...
    async_1.waterfall([
        function (cb) {
            if (vld.hasFields(body, fieldList, cb) && vld.checkFieldLengths(body, lengths, cb) && vld.hasOnlyFields(body, fieldList, cb))
        ...
        function (foundCnvs, fields, cb) {
            if (foundCnvs.length) {
                ...
                foundCnvs[0].lastMessage =
                    found.lastMessage && found.lastMessage.getTime();
...
let queryMsgs = (req, cnn, mySQLDate, cb) => {
