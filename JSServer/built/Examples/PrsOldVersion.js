...
router.get('/', function (req, res) {
    ...
    if (email)
        cnn.query('select id, email from Person where email = ?', [email], function (err, result) {
            if (err) {
...
router.post('/', function (req, res) {
    ...
    if (noPerm)
        ...
        cnn.query(qry = 'select * from Person where email = ?', body.email, function (err, dupEmail) {
            if (err) {
                cnn.destroy();
                ...
                cnn.query(qry = 'insert into Person set ?', body, function (err, insRes) {
                    cnn.destroy();
                    if (err)
                        res.status(500).json("Failed query " + qry);
                    else
