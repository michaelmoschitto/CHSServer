      ...
      [
         function(cb: queryCallback) {
            if (req.session) {
               if (req.query.owner)
                  req.cnn.chkQry(
                   'select distinct Conversation.id, title, ownerId, ' +
                   'lastMessage from Conversation where ownerId = ?',
                   [req.query.owner], cb);
               else
                  req.cnn.chkQry(
                   'select distinct Conversation.id, title, ownerId, '+
                   'lastMessage from Conversation left join ' +
                   'Message on Conversation.id = cnvId', [], cb);
            }
         },

         function(cnvsRet: Conversation[], fields: any, cb: queryCallback) {
            cnvsRet.forEach((cnvs: Conversation) =>
             (cnvs.lastMessage = (cnvs.lastMessage as Date) &&
...
router.post('/', function(req: Request, res: Response) {
   ...
   waterfall(
      [
         function(cb: queryCallback) {
            if (vld.hasFields(body, fieldList, cb) && vld.checkFieldLengths(
             body, lengths, cb) && vld.hasOnlyFields(body, fieldList, cb))
               cnn.chkQry(
         ...
         function(insRes: { insertId: number }, fields: any, 
          cb: queryCallback) {
      ...
      [
         ...
         function(foundCnvs: Conversation[], fields: any, cb: queryCallback) {
            if (foundCnvs.length) {
               ...
               foundCnvs[0].lastMessage =
                found.lastMessage && (found.lastMessage as Date).getTime();
         ...
         function(cb: queryCallback) {
            if (
             ...
             vld.hasOnlyFields(body, fieldList, cb)
               ...
               cnn.chkQry(
                'select * from Conversation where id = ?', [cnvId], cb);
         },
...
let queryMsgs = (req: Request, cnn: PoolConnection, mySQLDate: Date, 
 cb: queryCallback) => {
   if (req.query.num && req.query.dateTime)
      cnn.chkQry(
       'select Message.id, prsId, whenMade, ' +
       ...
       'limit ?', [req.params.cnvId, mySQLDate,
       parseInt(req.query.num as string)], cb);
   ...
   waterfall(
      [
         ...
         function(foundCnvs: Conversation[], fields: any, cb: queryCallback) {
            ...
            if (foundCnvs.length) {
               queryMsgs(req, cnn, mySQLDate, cb);
   ...
   waterfall(
      [
         ...
         function(result: { insertId: number }, fields: any, cb: queryCallback){
            cnn.chkQry(
             'update Conversation set lastMessage = ? where id = ?',
